...
def test_strategy_round_trip_keeps_tiers_assignments_notes_and_maximum_price_percentage(
    session_factory,
):
    with session_factory() as session:
        ...
        strategy.update_player(player.external_id, player.role, tier_id, "Primary target", 15.5)
        session.add(player)
    ...
    with session_factory() as session:
        ...
        assert saved.entries[0].maximum_price_percentage == 15.5
        assert list(saved.events) == []
